...
[source]
----
oc patch bc/blue -p '{"spec":{"source":{"git":{"uri":"https://github.com/<your username>/bluegreen.git","ref": "master"}}}}' --type=strategic -n bluegreen-{{USER_NAME}}
oc patch bc/green -p '{"spec":{"source":{"git":{"uri":"https://github.com/<your username>/bluegreen.git","ref": "green"}}}}' --type=strategic -n bluegreen-{{USER_NAME}}
----
...
oc deploy myapp --cancel -n testing-{{USER_NAME}}
...
and from the top level, select the github organization entry (in the *All* tab) with *your github user name* -> *Confiugure*. And under the *Projects -> Advanced button* -> Check the `Build origin PRs (merged with base branch)` checkbox and select *Save*.
